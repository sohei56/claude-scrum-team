...
- **No-schema SSOT siblings.** Several runtime files live under
  ...
  `.scrum/test-results.json` (smoke-test summary), `.scrum/sprint-history.json` (rolling list of completed Sprints),
  `.scrum/improvements.json` (retrospective backlog), and
  ...
  script, smoke-test wrapper, retrospective skill), so the guard
  never intercepts them. Schemas are out of scope until a
  reader-side bug demands them — see `MIGRATION-scrum-state-tools.md`
  § Known gaps.
