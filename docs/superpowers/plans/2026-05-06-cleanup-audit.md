...
## Completed: T5 batch (Cosmetic)

| # | 完了内容 | Files |
|---|---|---|
| T5-2 | `dashboard-event.sh::shorten_id` を Helpers ブロックへ移動（旧位置 = Main 内部） | `hooks/dashboard-event.sh` |
| T5-3 | `pbi-pipeline/SKILL.md` sub-agent table → 4-bullet list (parallelism は ‖ 記号で表現) | `skills/pbi-pipeline/SKILL.md` |
| T5-4 | `requirements-sprint/SKILL.md` に Roles section 追加 (SM step / Developer step 分担) | `skills/requirements-sprint/SKILL.md` |
| T5-5 | `pbi-state.schema.json` description に `merge_failure.kind` (bare) vs `escalation_reason` (`merge_*` prefix) 命名規則を一行注記 | `docs/contracts/scrum-state/pbi-state.schema.json` |
| T5-6 | `setup-user.sh` PostToolUse `Agent` matcher 検証 → **valid, no action**: `dashboard-event.sh:274` で `agent_spawn` communications message を生成（`SubagentStart` の dashboard event とは別役割） | (検証のみ) |
| T5-7 | `dashboard-event.sh` に `FileChanged\|file_changed` case 追加 → settings.json registration が default fall-through で `status_transition` 化していた問題を解消、`file_changed` event + external `file_change` comms message を発行 | `hooks/dashboard-event.sh` |

### Deferred

- **T5-1** (`hooks/lib/codex-invoke.sh` を移動): 移動は agents/*.md (3 files) + setup-user.sh deploy path + tests (2) + architecture.md と blast radius が広く、純粋 cosmetic 範囲を超える。別途 follow-up タスクで扱う。`hooks/lib/` に置かれたままだが、コメントヘッダで agent helper である旨を明記済 (T4 段階で確認)。

### Verification (T5)

| 項目 | 結果 |
|---|---|
| `bats --recursive tests/unit/ tests/lint/ tests/integration/` | **305 pass, 0 fail, 6 skip** |
| `shellcheck` (`scrum-start.sh`, `scripts/`, `scripts/lib/`, `scripts/scrum/`, `scripts/scrum/lib/`, `hooks/`, `hooks/lib/`) | **clean** |
| `ruff check dashboard/` + `ruff format --check` | **clean** |
...
- 2026-05-07: T5 batch (T5-2/3/4/5/6/7; T5-1 deferred) 完了、4 commits + plan 更新、bats 305 pass + shellcheck + ruff clean。T5-6 は検証のみ (Agent matcher は intentional)。T5-7 で settings.json の `FileChanged` registration が dispatcher 不在のため `status_transition` に格下げされていた潜在バグを併せて修正
