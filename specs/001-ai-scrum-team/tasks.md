...
- [x] T001 Create project directory structure per plan.md: `agents/`, `skills/` (13 subdirectories: `sprint-planning/`, `spawn-teammates/`, `install-subagents/`, `design-phase/`, `implementation-phase/`, `cross-review/`, `sprint-review/`, `retrospective/`, `requirements-sprint/`, `integration-sprint/`, `backlog-refinement/`, `change-process/`, `scaffold-design-spec/`), `hooks/`, `dashboard/`, `scripts/`, `tests/{unit,lint,integration,fixtures,test_helper}/`
...
- [x] T006 [P] Create Developer teammate template `agents/developer.md` with YAML frontmatter: `skills: [install-subagents]`, lifecycle steps (spawn → PBI assignment → read improvements → install sub-agents → design → implement → review → terminate). Per agent-interfaces.md § Developer — responsibilities: FR-002, FR-004, FR-009, FR-012, FR-017, FR-019
...
- [x] T031 [P] Create skill definition lint tests `tests/lint/skill-frontmatter.bats`: validate all 13 `skills/*/SKILL.md` files — YAML frontmatter present with required fields, `## Inputs` and `## Outputs` sections exist in body, `disable-model-invocation: true` set. Use yq for frontmatter and grep for body sections
