...
1. **Prepare**
   - `scripts/scrum/update-pbi-state.sh "$PBI_ID" design_round "$n" design_status pending`
   - `scripts/scrum/append-pbi-log.sh "$PBI_ID" design "$n" start —`
...
2. **Step 1: Spawn pbi-designer** (single Agent call)
   ...
   - `scripts/scrum/update-pbi-state.sh "$PBI_ID" design_status in_review`
...
4. **Step 3: Termination gate** (see termination-gates.md)
   - **Success**: design-reviewer verdict == PASS
     - `scripts/scrum/update-pbi-state.sh "$PBI_ID" design_status pass phase impl_ut impl_round 0`
     - `scripts/scrum/append-pbi-log.sh "$PBI_ID" design "$n" gate "success → impl_ut"`
   ...
   - **Stagnation / Divergence / Hard cap**: escalate
     ...
     - `scripts/scrum/append-pbi-log.sh "$PBI_ID" design "$n" gate "escalate → <reason>"`
     - Notify SM (see `escalation-notify` snippet below)
   - **Other FAIL**: review-r{n}.md becomes input to Round n+1
     - `scripts/scrum/append-pbi-log.sh "$PBI_ID" design "$n" gate "fail → round $((n+1))"`
     - Increment n, recurse.
