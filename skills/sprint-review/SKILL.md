...
| 4. Demo EVERY completed PBI      | SM sends `[sprint-<N>] PO_DECISION_REQUEST kind=demo_acceptance options=[pass,fail,waive] recommendation=pass` with the completed-PBI list, then invokes the PO via the `po-acceptance` skill (`mode=demo`). The PO launches and operates the app, verifies each AC by runnable command, and returns one `kind=demo_acceptance` decision per PBI. `fail` routes to step 9. |
...
4. **Demo EVERY completed PBI (mandatory)**:
   ...
   - **po_mode=agent**: skip step 4a–d. Send `[sprint-<N>] PO_DECISION_REQUEST kind=demo_acceptance options=[pass,fail,waive] recommendation=pass pbis=[<list>]`, invoke `po-acceptance` skill mode=demo. PO operates the app itself, returns one `kind=demo_acceptance` decision per PBI plus the aggregated `PO_ACCEPTANCE_REPORT`. fail → step 9 defect route.
